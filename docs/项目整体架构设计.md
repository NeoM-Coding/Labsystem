...
```mermaid
flowchart LR
    web["web HTTP 入口"] --> mqttApi["mqtt-api 契约与协议"]
    web --> common["common 公共模型"]

    mqtt["mqtt 服务实现"] --> mqttApi
    mqtt --> common
    mqtt --> redis["redis RedisBus"]

    rule["rule-engine"] --> mqttApi
    rule --> common
    rule --> redis
...
- `mqtt-api` 放 MQTT 分布式接口契约、入参/出参 DTO 和二进制命令协议。
- `common` 放跨模块稳定共享的模型、协议、工具和基础配置。
- 服务实现模块依赖 `mqtt-api`，不要让 `mqtt-api` 反向依赖服务实现。
...
flowchart TB
    ...
    web -->|Dubbo Reference| apiIo["mqtt-api: MqttIo / MqttPollCo"]
    ...
    mqttApi["mqtt-api CommandLine / ByteUtil"] --> mqttManager
    deviceDomain["device-domain Device / Gateway"] --> mqttManager
```
...
8. `ActiveQueue<Poll<MqttTask>>` 防止同一设备重复注册轮询。
...
- `mqtt-api` 模块统一承载 MQTT 分布式契约和命令协议，Dubbo 边界清晰。
- `common` 承载协议、模型、工具，复用边界比较明确。
- MQTT 同一网关串行发送，符合 RS485 设备链路特点。
- `ActiveQueue` 的 active set 语义贴合“防重复轮询”的业务目标；规则引擎改由 `AsyncRuntimeScheduler` 提供 runtime 单飞与 dirty 合并。
...
- `mqtt-api` 当前仍依赖 `device-domain` 中的设备类型，契约层与设备领域存在明确的单向依赖。
